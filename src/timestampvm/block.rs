...
impl State {
	...
	pub fn get_last_accepted_block_id(&mut self) -> Result<Option<Id>, LandslideError> {
		self.block_db.get(LAST_ACCEPTED_BLOCK_ID_KEY)?
				.map_or(Ok(None), |last_accepted_block_id_bytes| Ok(Some(Id::new(last_accepted_block_id_bytes.as_ref())?)))
	}

	pub fn set_last_accepted_block_id(&self, id: Id) -> Result<(), LandslideError> {
		self.block_db.insert(LAST_ACCEPTED_BLOCK_ID_KEY, id.as_ref())?;

		Ok(())
	}

    pub fn is_state_initialized(&mut self) -> Result<bool, LandslideError> {
		self.block_db.get(STATE_INITIALIZED_KEY)?
				.map_or(Ok(false), |_| Ok(true))
	}

	pub fn set_state_initialized(&self) -> Result<(), LandslideError> {
		self.block_db.insert(STATE_INITIALIZED_KEY, STATE_INITIALIZED_VALUE)?;
